...
def list_documents():
    ...
    return jsonify(vectorstore.list_documents(qdrant_client))
...
def upload():
    ...
    for file_storage in files:
        try:
            summary = upload_service.ingest_upload(qdrant_client, embedding_model, file_storage)
...
# Runs unconditionally at import time - NOT gated behind
# `if __name__ == "__main__"`. That guard only fires when this file
# is executed directly (`python app.py`); gunicorn instead *imports*
# this module (`gunicorn app:app`), so anything needed for the app to
# actually work has to run here, at module level, or it silently
# never happens and every route relying on qdrant_client stays
# permanently gated behind the "still starting up" 503.
...
if pdf_path and os.path.exists(pdf_path):
    ...
    upload_service.ingest_saved_file(qdrant_client, embedding_model, pdf_path)

print("\n🚀 API ready at http://localhost:8000")

if __name__ == "__main__":
    # Only the Flask dev server itself is guarded - gunicorn (used in
    # Docker) runs its own server and must not also trigger this one.
